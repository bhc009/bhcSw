...
CBhImage::CBhImage()
{
	...
	OutputDebugString(_T("BH's image... v2.001\n"));
}
...
//
//////////////////////////////////////////////////////////////////////////
void CBhImage::SetColorImage(BYTE *pRed, BYTE *pGreen, BYTE *pBlue, int iWidth, int iHeight)
{
	Create( iWidth, iHeight, 24, 0 );
	...
	for( int y=0 ; y<iHeight ; y++ )
	{
		int iPosY_src = y*iWidth;
		int iPosY_Dst = y * GetWidthStep();
		for( int x=0 ; x<iWidth ; x ++ )
		{
			pDst[ 3*x + iPosY_Dst ]		= pBlue[ x + iPosY_src ];
			pDst[ 3*x + 1 + iPosY_Dst ]	= pGreen[ x + iPosY_src ];
			pDst[ 3*x + 2 + iPosY_Dst ]	= pRed[ x + iPosY_src ];
		}
	}
